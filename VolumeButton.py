...
from Global import generate_new_arrslice, i18n
    ...
    def _clicked(self):
        ...
        if mode == 1:
            for code in data:
                ...
                arrslice = generate_new_arrslice(chartrows, centralwidget.arrslice, centralwidget.maxlen)
                chartrows = chartrows[arrslice]
                if chartrows[-1][1] == 0: continue
                if len(chartrows) < 10: continue
                arrv = []
                for chartrow in chartrows:
                    v = chartrow[1]
                    if v > 0:
                        arrv.append(v)
                if len(arrv) < 5: continue
                todayv = arrv[-1]
                arrv.sort(reverse=True)
                bigtop = arrv[:Constants.VOLUMETOPN]
                if todayv < bigtop[-1]: continue
