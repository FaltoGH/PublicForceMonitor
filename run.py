...
infonamelist = ["평균거래단가", "보유비중증감", "영향력", "방향", "RS", "Bollinger"]
...
class MainWindow(QMainWindow):
    ...
    def checkfile(self):
        dirlist = os.listdir()
        if ("atad.p" in dirlist) and (Constants.PATH_DATAP in dirlist):
            mtime0 = os.path.getmtime("atad.p")
            timestr0 = Util.float2time(mtime0)
            mtime1 = os.path.getmtime(Constants.PATH_DATAP)
        ...
        for code, name, date, score, index, price in codenamedatescoreindexprice:
            ...
            if code in self.bookmarks:
                for x in [code, name, date, score]:
                    ...
                    item.vline = index
                    item.hline = price
                    item.setBackground(bookmarkcolor)
                    row.append(item)
